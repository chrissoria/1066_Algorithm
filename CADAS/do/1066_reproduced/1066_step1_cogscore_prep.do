...
* Pentagon drawing
* cs_32 is already the cleaned score (Ty's manual re-scoring prioritized, interviewer
* score as fallback) -- the swap happens in Read/Cog_Scoring_Read.do, so both branches
* below read the same cleaned source.
if "$run_pre_prep" == "yes" {
...
if "$run_pre_prep" == "yes" {
    * Immediate word recall (3 trials summed) - use recoded versions
    gen wordimm = c_11_recoded + c_12_recoded + c_13_recoded

    * Delayed word recall (3 trials summed) - use recoded versions
    gen worddel = c_21_recoded + c_22_recoded + c_23_recoded
}
else {
...
if "$run_pre_prep" == "yes" {
    gen paper = c_27_recoded + c_28_recoded + c_29_recoded
}
else {
...
if "$run_pre_prep" == "yes" {
    * Recode story elements to binary using recoded versions
    foreach var in c_66a_recoded c_66b_recoded c_66c_recoded c_66d_recoded c_66e_recoded c_66f_recoded {
        replace `var' = 1 if `var' == 0 | `var' == 1
        replace `var' = 0 if `var' == 2
    }

    gen story = c_66a_recoded + c_66b_recoded + c_66c_recoded + c_66d_recoded + c_66e_recoded + c_66f_recoded
}
else {
...
if "$run_pre_prep" == "yes" {
    gen learn1 = c_33_1_recoded + c_33_2_recoded + c_33_3_recoded + c_33_4_recoded + c_33_5_recoded + c_33_6_recoded + c_33_7_recoded + c_33_8_recoded + c_33_9_recoded + c_33_10_recoded
}
else {
...
* Learning trial 2
if "$run_pre_prep" == "yes" {
    gen learn2 = c_34_1_recoded + c_34_2_recoded + c_34_3_recoded + c_34_4_recoded + c_34_5_recoded + c_34_6_recoded + c_34_7_recoded + c_34_8_recoded + c_34_9_recoded + c_34_10_recoded
}
else {
...
* Learning trial 3
if "$run_pre_prep" == "yes" {
    gen learn3 = c_35_1_recoded + c_35_2_recoded + c_35_3_recoded + c_35_4_recoded + c_35_5_recoded + c_35_6_recoded + c_35_7_recoded + c_35_8_recoded + c_35_9_recoded + c_35_10_recoded
}
else {
...
* Delayed recall (10 words) — handled via imputation, not recoded
gen recall = c_63_1 + c_63_2 + c_63_3 + c_63_4 + c_63_5 + c_63_6 + c_63_7 + c_63_8 + c_63_9 + c_63_10
rename c_63_a recall_refuse

* Name recall
if "$run_pre_prep" == "yes" {
    rename c_0_recoded name
    rename c_65_recoded nrecall
}
else {
    rename c_0 name
    rename c_65 nrecall
...
* Set to missing if refused (only needed when pre-prep hasn't already handled refusals)
if "$run_pre_prep" != "yes" {
...
* Only needed when pre-prep hasn't already handled these
*-------------------------------------------------------------------------------
...
* Animals: handle out of range values (only when pre-prep hasn't already cleaned + imputed)
if "$run_pre_prep" != "yes" {
