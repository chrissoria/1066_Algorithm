...
1. to adjust the algorithm and test whether the adjustments brings it in line a bit closer with the Cuban diagnosis
2. to identify the adjustments that improve its alignment with the cuban diagnosis

I will try to following:

1. convert continuous variables into categorical based on quintile distribution
...
* 1066 but with learn1 slotted in instead of recall
gen dem1066_score_learn1 = exp(8.486511 -.4001659 * cogscore + .5024221 * relscore -.6997248 * learn1) / (1 + exp(8.486511 -.4001659 * cogscore + .5024221 * relscore -.6997248 * learn1))

gen dem1066_learn1 = .
replace dem1066_learn1 = 1 if dem1066_score_learn1 >= .5 & dem1066_score_learn1 != .
replace dem1066_learn1 = 0 if dem1066_score_learn1 <.5 & dem1066_score_learn1 != .

* 1066 but with learn3 slotted in instead of recall
gen dem1066_score_learn3 = exp(8.486511 -.4001659 * cogscore + .5024221 * relscore -.6997248 * learn3) / (1 + exp(8.486511 -.4001659 * cogscore + .5024221 * relscore -.6997248 * learn3))

gen dem1066_learn3 = .
replace dem1066_learn3 = 1 if dem1066_score_learn3 >= .5 & dem1066_score_learn3 != .
replace dem1066_learn3 = 0 if dem1066_score_learn3 <.5 & dem1066_score_learn3 != .

*linear model

gen dem1066_score_linear = .6936593 -.019811 * cogscore + .0319944 * relscore -.0196586 * learn1

gen dem1066_linear = .
replace dem1066_linear = 1 if dem1066_score_linear >= .5 & dem1066_score_linear != .
replace dem1066_linear = 0 if dem1066_score_linear <.5 & dem1066_score_linear != .

*categorical version using 10/66 own categories
gen dem1066_score_categorical_orig = exp(-17.71921 + 2.76109 * bcogscor + 1.836585 * brelscor + 2.126105 * bdelay) / (1 + exp(-17.71921 + 2.76109 * bcogscor + 1.836585 * brelscor + 2.126105 * bdelay))

gen dem1066_categorical_orig = .
replace dem1066_categorical_orig = 1 if dem1066_score_categorical_orig >= .5 & dem1066_score_categorical_orig != .
replace dem1066_categorical_orig = 0 if dem1066_score_categorical_orig <.5 & dem1066_score_categorical_orig != .

*categorical version using 10/66 own categories but quints from cadas
gen dem1066_score_orig_quint = exp(-17.71921 + 2.76109 * bcogscor_quint + 1.836585 * brelscor_quint + 2.126105 * bdelay_quint) / (1 + exp(-17.71921 + 2.76109 * bcogscor_quint + 1.836585 * brelscor_quint + 2.126105 * bdelay_quint))

gen dem1066_orig_quint = .
replace dem1066_orig_quint = 1 if dem1066_score_orig_quint >= .5 & dem1066_score_orig_quint != .
replace dem1066_orig_quint = 0 if dem1066_score_orig_quint <.5 & dem1066_score_orig_quint != .


*categorical version using quintiles based on our own data
gen dem1066_score_quint = exp(-3.99328 - 2.147228 * ncogscor_quint + 1.857513 * nrelscor_quint - 1.312263 * ndelay_quint) / (1 + exp(-3.99328 - 2.147228 * ncogscor_quint + 1.857513 * nrelscor_quint - 1.312263 * ndelay_quint))

gen dem1066_categorical_quint = .
replace dem1066_categorical_quint = 1 if dem1066_score_quint >= .5 & dem1066_score_quint != .
replace dem1066_categorical_quint = 0 if dem1066_score_quint <.5 & dem1066_score_quint != .

*down-weighted relscore
gen dem1066_score_downweighted = exp(8.486511 - .4001659 * cogscore + (.5024221*.7) * relscore - .6997248 * recall) / (1 + exp(8.486511 - .4001659 * cogscore + (.5024221*.7) * relscore - .6997248 * recall))

gen dem1066_rel_downweighted = .
replace dem1066_rel_downweighted = 1 if dem1066_score_downweighted >= .5 & dem1066_score_downweighted != .
replace dem1066_rel_downweighted = 0 if dem1066_score_downweighted <.5 & dem1066_score_downweighted != .

*binary relscore
gen rel_binary = (relscore > 4) if relscore != .
gen dem1066_score_binary = exp(9.736947 - .426476 * cogscore + 3.41575 * rel_binary - .6517573 * recall) / (1 + exp(9.736947 - .426476 * cogscore + 3.41575 * rel_binary - .6517573 * recall))

gen dem1066_rel_binary = .
replace dem1066_rel_binary = 1 if dem1066_score_binary >= .5 & dem1066_score_binary != .
replace dem1066_rel_binary = 0 if dem1066_score_binary <.5 & dem1066_score_binary != .

*quints from cadas
gen dem1066_score_cadas = exp(-3.190348 - 2.113413 * ncogscor_quint + 1.637074 * nrelscor_quint - 1.119034 * ndelay_quint) / (1 + exp(-3.190348 - 2.113413 * ncogscor_quint + 1.637074 * nrelscor_quint - 1.119034 * ndelay_quint))

gen dem1066_cadas_quint = .
replace dem1066_cadas_quint = 1 if dem1066_score_cadas >= .5 & dem1066_score_cadas != .
replace dem1066_cadas_quint = 0 if dem1066_score_cadas <.5 & dem1066_score_cadas != .

*cog components
gen dem1066_cog_comp_prob = exp(3.893365 + 3.724723*nametot - .5088708*count - 1.091296*animtot + 1.470641*wordtot1 - 1.080204*wordtot2 + .4929788*papertot + .2299829*storytot + .5421012*relscore - .7007926*recall) / (1 + exp(3.893365 + 3.724723*nametot - .5088708*count - 1.091296*animtot + 1.470641*wordtot1 - 1.080204*wordtot2 + .4929788*papertot + .2299829*storytot + .5421012*relscore - .7007926*recall))

gen dem1066_cog_comp = .
replace dem1066_cog_comp = 1 if dem1066_cog_comp_prob >= .5 & dem1066_cog_comp_prob != .
replace dem1066_cog_comp = 0 if dem1066_cog_comp_prob <.5 & dem1066_cog_comp_prob != .

*interacting cog and relscores
gen cog_rel_interact = cogscore * relscore

gen dem1066_interact_prob = exp(3.902774 - .2340031*cogscore + 2.067158*relscore - .687096*recall - .0565224*(cogscore*relscore)) / (1 + exp(3.902774 - .2340031*cogscore + 2.067158*relscore - .687096*recall - .0565224*(cogscore*relscore)))

gen dem1066_interact = .
replace dem1066_interact = 1 if dem1066_interact_prob >= .5 & dem1066_interact_prob != .
replace dem1066_interact = 0 if dem1066_interact_prob <.5 & dem1066_interact_prob != .


* now let's check accuracy for each one against CDR
local preds dem1066 dem1066_linear dem1066_categorical_orig dem1066_categorical_quint dem1066_rel_downweighted dem1066_rel_binary dem1066_cadas_quint dem1066_orig_quint dem1066_cog_comp dem1066_interact dem1066_learn1 dem1066_learn3

preserve
tempfile results
postfile handle str30 var sensitivity specificity accuracy auc TP TN FP FN mean_var mean_CDR overall_mean using `results', replace
...
foreach var of local preds {
    ...
    * Now restrict to non-missing CDR for all below
    ...
    roctab cuba_CDR_binary `var' if cuba_CDR_binary != ., nograph
    local auc = r(area)
    quietly summarize cuba_CDR_binary
    local mean_CDR = r(mean)

    post handle ("`var'") (`sens') (`spec') (`acc') (`auc') (`TP') (`TN') (`FP') (`FN') (`mean_var') (`mean_CDR') (`overall_mean')
}
postclose handle
...
foreach var of local preds {
    ...
    * Now restrict to non-missing CDR for all below
    ...
    local spec = (`TN'/(`TN'+`FP'))
    local acc = ((`TP'+`TN')/(`TP'+`TN'+`FP'+`FN'))
    roctab finalniaa_int_binary `var' if finalniaa_int_binary != ., nograph
    local auc = r(area)
    quietly summarize finalniaa_int_binary
    local mean_CDR = r(mean)
...
keep pid cuba_CDR_binary relscore cogscore nametot count animals_diss animals animtot wordtot1 wordtot2 papertot c_66a c_66b c_66c c_66d c_66e c_66f story storytot pencil watch chair shoes knuckle elbow should bridge hammer pray chemist repeat town street store address month day year nod point pentag pentag_diss animals animals_diss chief longmem season c_11 c_12 c_13 wordimm c_21 c_22 c_23 worddel misstot activ mental memory put kept frdname famname convers wordfind wordwrg past lastsee lastday orient lostout lostin chores hobby money change reason feed dress toilet recall dem1066 dem1066_linear dem1066_categorical_orig dem1066_categorical_quint dem1066_rel_downweighted dem1066_rel_binary dem1066_cadas_quint dem1066_orig_quint S finalniaa_int_binary immed learn1 learn2 learn3 bcogscor_quint  ncogscor_quint

order pid cuba_CDR_binary relscore cogscore nametot count animals_diss animals animtot wordtot1 wordtot2 papertot c_66a c_66b c_66c c_66d c_66e c_66f story storytot pencil watch chair shoes knuckle elbow should bridge hammer pray chemist repeat town street store address month day year nod point pentag pentag_diss animals animals_diss chief longmem season c_11 c_12 c_13 wordimm c_21 c_22 c_23 worddel misstot activ mental memory put kept frdname famname convers wordfind wordwrg past lastsee lastday orient lostout lostin chores hobby money change reason feed dress toilet recall dem1066 dem1066_linear dem1066_categorical_orig dem1066_categorical_quint dem1066_rel_downweighted dem1066_rel_binary dem1066_cadas_quint dem1066_orig_quint S finalniaa_int_binary immed learn1 learn2 learn3 bcogscor_quint  ncogscor_quint

preserve
* Define the variables as a local macro
local vars relscore cogscore nametot count animals animtot wordtot1 wordtot2 papertot story storytot pencil watch chair shoes knuckle elbow should bridge hammer pray chemist repeat town street store address month day year nod point pentag chief longmem season wordimm worddel misstot activ mental memory put kept frdname famname convers wordfind wordwrg past lastsee lastday orient lostout lostin chores hobby money change reason feed dress toilet recall dem1066 dem1066_linear dem1066_categorical_orig dem1066_categorical_quint dem1066_rel_downweighted dem1066_rel_binary dem1066_cadas_quint dem1066_orig_quint S immed learn1 learn2 learn3 bcogscor_quint ncogscor_quint
