...
gen dem1066_score_linear = .6936593 -.019811 * cogscore + .0319944 * relscore -.0196586 * recall
...
*now, let's run sensitivity, specificity, accuracy tests against the cuban diagnosis cuba_CDR_binary
local preds dem1066 dem1066_linear dem1066_categorical_orig dem1066_categorical_quint dem1066_rel_downweighted dem1066_rel_binary dem1066_cadas_quint dem1066_orig_quint

preserve
tempfile results
postfile handle str30 var sensitivity specificity accuracy auc TP TN FP FN using `results', replace

foreach var of local preds {
    // Get confusion matrix
    tab cuba_CDR_binary `var', matcell(freq)
    local TP = freq[2,2]
    local TN = freq[1,1]
    local FP = freq[1,2]
    local FN = freq[2,1]
    ...
    // Save r(area) = AUC after roctab
    roctab cuba_CDR_binary `var', nograph
    local auc = r(area)

    post handle ("`var'") (`sens') (`spec') (`acc') (`auc') (`TP') (`TN') (`FP') (`FN')
}
postclose handle
...
foreach var of local preds {
    // Get confusion matrix
    tab finalniaa_int_binary `var', matcell(freq)
    local TP = freq[2,2]
    local TN = freq[1,1]
    local FP = freq[1,2]
    local FN = freq[2,1]
    ...
    // Save r(area) = AUC after roctab
    roctab finalniaa_int_binary `var', nograph
    local auc = r(area)
...
keep pid cuba_CDR_binary relscore cogscore nametot count animals_diss animals animtot wordtot1 wordtot2 papertot c_66a c_66b c_66c c_66d c_66e c_66f story storytot pencil watch chair shoes knuckle elbow should bridge hammer pray chemist repeat town street store address month day year nod point pentag pentag_diss animals animals_diss chief longmem season c_11 c_12 c_13 wordimm c_21 c_22 c_23 worddel misstot activ mental memory put kept frdname famname convers wordfind wordwrg past lastsee lastday orient lostout lostin chores hobby money change reason feed dress toilet recall dem1066_score dem1066 S

order pid cuba_CDR_binary relscore cogscore nametot count animals_diss animals animtot wordtot1 wordtot2 papertot c_66a c_66b c_66c c_66d c_66e c_66f story storytot pencil watch chair shoes knuckle elbow should bridge hammer pray chemist repeat town street store address month day year nod point pentag pentag_diss animals animals_diss chief longmem season c_11 c_12 c_13 wordimm c_21 c_22 c_23 worddel misstot activ mental memory put kept frdname famname convers wordfind wordwrg past lastsee lastday orient lostout lostin chores hobby money change reason feed dress toilet recall dem1066_score dem1066 S

preserve
* Define the variables as a local macro
local vars relscore cogscore nametot count animals animtot wordtot1 wordtot2 papertot story storytot pencil watch chair shoes knuckle elbow should bridge hammer pray chemist repeat town street store address month day year nod point pentag chief longmem season wordimm worddel misstot activ mental memory put kept frdname famname convers wordfind wordwrg past lastsee lastday orient lostout lostin chores hobby money change reason feed dress toilet recall dem1066_score dem1066 S
